...
async function verifyContentful() {
  try {
    ...
    // Hero Content
    const heroes = await environment.getEntries({ content_type: 'heroContent', limit: 10 });
    console.log(`✅ Hero Content: ${heroes.items.length} entrada(s)`);
    if (heroes.items.length > 0) {
      const hero = heroes.items[0];
      console.log(`   - Headline: ${hero.fields.headline?.['en-US']?.substring(0, 50)}...`);
      console.log(`   - Tagline: ${hero.fields.tagline?.['en-US']}`);
      console.log(`   - CTA: ${hero.fields.ctaButtonText?.['en-US']}`);
    }

    // Gargalos
    const gargalos = await environment.getEntries({ content_type: 'gargalo', limit: 100 });
    ...
    // Methodology Steps
    const steps = await environment.getEntries({ content_type: 'methodologyStep', limit: 100 });
    console.log(`\n✅ Methodology Steps: ${steps.items.length} entrada(s)`);
    steps.items.forEach((item, idx) => {
      console.log(`   ${item.fields.number?.['en-US']}. ${item.fields.title?.['en-US']}`);
    });

    // Acceleration Content
    const accelerations = await environment.getEntries({ content_type: 'accelerationContent', limit: 10 });
    ...
    // Solutions
    const solutions = await environment.getEntries({ content_type: 'solution', limit: 100 });
    ...
    // Footer Content
    const footers = await environment.getEntries({ content_type: 'footerContent', limit: 10 });
    ...
    // Contact Content
    const contacts = await environment.getEntries({ content_type: 'contactContent', limit: 10 });
    ...
    console.log(`   • Total de Entradas: ${heroes.items.length + gargalos.items.length + steps.items.length + accelerations.items.length + solutions.items.length + footers.items.length + contacts.items.length}`);
