...
if (!SPACE_ID || !MANAGEMENT_TOKEN) {
  console.error('❌ Erro: Configure CONTENTFUL_SPACE_ID e CONTENTFUL_MANAGEMENT_TOKEN no .env.local');
  process.exit(1);
}
...
async function setupContentful() {
  try {
    ...
    const heroContentType = await environment.createContentTypeWithId('heroContent', {
      name: 'Hero Content',
    ...
    const accelerationContentType = await environment.createContentTypeWithId('accelerationContent', {
    ...
    const solutionContentType = await environment.createContentTypeWithId('solution', {
      name: 'Solution',
    ...
    const aboutContentType = await environment.createContentTypeWithId('aboutContent', {
      name: 'About Content',
    ...
    const contactContentType = await environment.createContentTypeWithId('contactContent', {
    ...
    const heroEntry = await environment.createEntry('heroContent', {
      fields: {
        ...
        subheadline: { 'en-US': 'Acelere suas vendas com metodologia comprovada e resultados mensuráveis' },
    ...
    const accelerationEntry = await environment.createEntry('accelerationContent', {
      fields: {
        title: { 'en-US': 'Aceleração Comercial' },
        description: { 'en-US': 'Transformamos sua operação comercial em uma máquina de vendas previsível e escalável' },
      },
    });
    ...
    const solutions = [
      {
        ...
        description: 'Análise profunda da sua operação para identificar gargalos e oportunidades de crescimento',
        order: 1,
      },
      {
        title: 'Implementação de Processos',
        description: 'Estruturação e otimização de processos comerciais com metodologia ágil',
        order: 2,
      },
      {
        title: 'Treinamento de Equipes',
        description: 'Capacitação prática de vendedores e gestores com técnicas comprovadas',
        order: 3,
      },
      {
        title: 'Mentoria Executiva',
        description: 'Acompanhamento estratégico de líderes para decisões assertivas',
        order: 4,
      },
    ];
    ...
    const aboutEntry = await environment.createEntry('aboutContent', {
      fields: {
        ...
        description: { 'en-US': 'Somos especialistas em aceleração comercial com metodologia própria e resultados comprovados' },
      },
    });
    ...
    const contactEntry = await environment.createEntry('contactContent', {
      fields: {
        ...
        instagram: { 'en-US': 'https://instagram.com/therightbox' },
      },
    });
    ...
    const blogEntry = await environment.createEntry('blogPost', {
      fields: {
        ...
        excerpt: { 'en-US': 'Descubra as estratégias que estão transformando equipes comerciais em máquinas de vendas' },
        content: {
          'en-US': {
            nodeType: 'document',
            data: {},
