...
async function cleanupDuplicates() {
  try {
    ...
    if (gargalos.items.length > 6) {
      console.log('⚠️  Duplicatas encontradas! Mantendo apenas as 6 primeiras...');
    ...
    const footers = await environment.getEntries({ content_type: 'footerContent', limit: 100 });
    ...
    const heroes = await environment.getEntries({ content_type: 'heroContent', limit: 100 });
    ...
    const accelerations = await environment.getEntries({ content_type: 'accelerationContent', limit: 100 });
    ...
    const contacts = await environment.getEntries({ content_type: 'contactContent', limit: 100 });
    ...
    console.log('\n🎉 LIMPEZA COMPLETA!\n');
    console.log('📋 Resumo:');
    console.log(`   • Gargalos: ${Math.min(gargalos.items.length, 6)} entradas mantidas`);
    console.log(`   • Methodology Steps: ${Math.min(steps.items.length, 6)} entradas mantidas`);
